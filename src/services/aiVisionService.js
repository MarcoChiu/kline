...
【特別警告與視覺重點】
1. **定位今日**：K線圖**最右側的最後一根 K 棒**代表「今日 (最新)」，請精準辨識它的顏色、實體長度與上下影線。
2. **多日組合比對**：絕大多數的 K 線型態（如吞噬、母子、烏雲罩頂）都需要比對前幾天的 K 棒。請務必仔細觀察「最右側倒數第 2 根與倒數第 3 根」的大小、相對高低位置、以及是否有跳空缺口，並將它們與最新一根 K 棒結合起來做綜合判定。

請嚴格執行以下工作流程：
Step 1: 視覺特徵提取 (價格、量能、最右側最新 K 棒型態、技術指標現況)
Step 2: 均線與趨勢判定 (判斷目前價格位於 MA20/MA60 之上或下？均線是多頭排列還是空頭排列？是否有均線糾結或乖離過大？)
...
JSON 格式定義：
{
  ...
  "prediction": {
    ...
    "sentimentSummary": "Step 3: 一句話總結目前籌碼、均線結構與共振預判",
